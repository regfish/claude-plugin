---
description: Answer questions about regfish itself from the official knowledge base — how something works, where a setting lives, what a process involves. Use for "how do I", "where do I find", "why does regfish" questions about accounts, contracts and regfish processes — AuthInfo and domain transfers, cancellation and auto-renewal, pricing and what things cost, invoices, payment and billing issues such as duplicate or unexpected charges, transfer lock, contact details, the company itself, or how the API and MCP tools work. Not for performing DNS or hosting changes — the dns and hosting skills cover those. Also when a customer asks in German ("wie kündige ich", "wo finde ich", "was kostet").
---
...
them into "the contract settings" or "show code". And do not invent steps
the articles do not mention — an article that ends after step 3 has three
steps.

## When the search comes back empty — or beside the point

Hits that do not actually answer the question count as empty: say what the
articles do cover, and pass the rest to support. Partial relevance is not a
license to fill the gap from general knowledge.

When there is truly nothing, say so. "The regfish knowledge base has nothing on this" is a complete and
honest answer; follow it with the support contact (support@regfish.de) so the
...
| both ("how do I transfer this domain away?") | `search_help` for the process, account tools for the state (is auto-renewal still on? `get_domain`; is DNSSEC active? `get_dnssec_status`). The transfer lock itself is not exposed via the API — it lives in the dash under „Mein Vertrag“ |
...
they work; the doing happens in the regfish dashboard. The one exception:
`request_auth_code` hands out the transfer code (on explicit request only) —
the transfer itself still runs at the gaining registrar.
